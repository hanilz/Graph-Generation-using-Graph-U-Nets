...
def main():
    ...
    cora_adjacency_matrix = np.load('{}/cora_adjacency_matrix.npy'.format(output_dir))

    if os.path.exists('{}/BA.npy'.format(output_dir)) and os.path.exists('{}/ER.npy'.format(output_dir)):
        data['BA'] = np.load('{}/BA.npy'.format(output_dir))
        data['ER'] = np.load('{}/ER.npy'.format(output_dir))
    else:
        n = cora_adjacency_matrix.shape[0]
        m = int(np.sum(cora_adjacency_matrix))
    ...
    data['cora'] = cora_adjacency_matrix
    # netgan = np.load('{}/netgan_network.npy'.format(output_dir))[1]
    # n = cora_adjacency_matrix.shape[0]
    # if netgan.shape[0] < n:
    #     Net = np.zeros((n, n))
    ...
    #     netgan = Net
    # data['netgan'] = netgan
    # data['music'] = np.load('{}/output_network.npy'.format(output_dir))

    savemat('{}/old_cora.mat'.format(output_dir), data)
